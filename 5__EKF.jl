...
params = Float64[1.0]
ode_prob = @time ODEProblem{true, SciMLBase.FullSpecialize}(fun, u₀ , tspan, params)
# ode_prob = @time ODEProblem{true, SciMLBase.FullSpecialize}(fun,df_u0_orig.u0, tspan)

@info "Trying a solve with default u₀"
sol = solve(ode_prob, TRBDF2(autodiff=false); saveat=Δt_step, reltol=reltol, abstol=abstol)
